...
#meth:2 最大迴圈
for (RowgetStockNum in 360:RowgetStockNumTotal) {
  getStockNum<-sapply(getStockNum_file[2],"[",RowgetStockNum)
  
  #大迴圈 START   SSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSSS
  #starttime<-proc.time()
  ...
  print(paste(getStockNum,body1[RowNumTotal_body1,3]," FINISHED !!!"))
}
#還原原先環境設定
#options(old.options)
...
# 
# 
# getStockNumFI<-mutate(getStockNumFI,"Name"=sub("東<U+78B1>",replacement="東碱",getStockNumFI$Name))
# 
# #RowgetStockNum=360 @ 2015Q3 "東<U+78B1>"
# mutate(body1,Name=)
# if (getStockNum==1708) {
#   for (i in 1:RowNumTotal_body1) {
#     if (body1[i,3]=="東<U+78B1>") {
#       body1[i,3]<-"東碱"
#     }
#   }
#   mutate(body1,"Name"=body1[,3])
# }