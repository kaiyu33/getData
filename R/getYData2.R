...
for (j in 1:length(ls(pattern = "^TW."))) {
  new_path00<-paste0(Upath,"Ydata/Ydata.",substr(ls(pattern = "^TW")[j],3,7),".csv")
  write.csv(get(ls(pattern = "^TW.")[j]),file = new_path00)
}
